...
export class StudentService {
  ...
  async getQuizzes(userId: string) {
    const invitations = await this.prisma.quizInvitation.findMany({
      // Drafts stay hidden from students until the admin publishes them.
      where: { user_id: userId, quiz: { status: QuizStatus.published } },
      include: {
        quiz: {
          select: { id: true, title: true, duration_minutes: true, starts_at: true, ends_at: true }
        }
      },
    ...
    return invitations.map(({ quiz }) => ({
      ...
      state: states.get(quiz.id) ?? "not_started"
    }));
  }
